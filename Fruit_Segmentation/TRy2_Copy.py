...
#Mimg=cv2.imread('E:/Python2/OverLap/Input/input (5).jpg')
...
#gray_blur = cv2.medianBlur(gray1, 1)
...
_, thresh = cv2.threshold(thresh1, 150, 255, cv2.THRESH_BINARY_INV)
# threshold
cv2.imshow('Thresh Image 2', thresh)
cv2.imwrite('directory/Thresh Image2.jpg', thresh)
kernel1 = cv2.getStructuringElement(cv2.MORPH_CROSS, (3, 3))
dilated = cv2.dilate(thresh1, kernel1, iterations=1)  # dilate
...
img14=np.ones(img.shape, dtype=np.uint8)*255

img6=Main
img9=np.ones(img.shape, dtype=np.uint8)*255


contours3, hier = cv2.findContours(dilated.copy(), cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
...
for cnt3 in contours3:
    ...
    cv2.drawContours(img9, [cnt3], -1, (0, 0, 255))
    ...
    if len(cnt3) > 145:
        cv2.drawContours(img14, [hull], -1, (0, 0, 255))
        x, y, w, h = cv2.boundingRect(hull)
        # # if w > 10 and h > 10:
        # # ------#-------print("%d %d  %d %d" % (x, y, w,h))
        # idx += 1
...
cv2.imshow('contours22', img14)
cv2.imwrite('directory/contour.jpg', img4)
cv2.imshow('kiti', img9)
cv2.imwrite('directory/kiti.jpg', img9)
